...
export interface OSSPIMImageVariant {
  url: string;
  width: number;
  height?: number;
  size?: number;
...
export interface Props extends HTMLAttributes<HTMLImageElement> {
  ...
  variants?: OSSPIMImageVariant[];
  loading?: 'eager' | 'lazy';
  _availableSizes?: number[];
  _availableFormats?: string[];
}

function getVariantSrc(variant: OSSPIMImageVariant): string {
...
export const Image: FC<Props> = ({ children, ...restOfAllProps }) => {
  ...
  let biggestImage: OSSPIMImageVariant = vars[0];
  if (hasVariants) {
    biggestImage = vars.reduce(function (
      acc: OSSPIMImageVariant,
      v: OSSPIMImageVariant
    ): OSSPIMImageVariant {
